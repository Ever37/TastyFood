...
<body>
	...
	<div class="section no-pad-bot" id="index-banner">
		...
		<div class="white">
			...
			<div class="container">
				...
				<c:if test="${comercios.size()!= 0}">


					<div class="col s12">
						<div class="material-table">
							<div class="table-header">
								<span class="table-title">Listado de comercios</span>
								<div class="actions">
									<a href="#"
							...
							<table id="tableData"
								...
								<thead>
									<tr>
										...
										<th width="20%">Apellido Nombre</th>
										<th class="center">Opciones</th>
									</tr>
								</thead>
								<tbody>
									<c:set var="comercio" value="0" />
									<c:forEach items="${comercios}" var="comercio">
										<tr>
											<td>${comercio.idComercio}</td>
											<td>${comercio.descripcion}</td>
											<td>${comercio.usuario.nombreUsuario}</td>
											<td>${comercio.usuario.apellido}
												${comercio.usuario.nombre}</td>
											<td class="center"><sec:authorize
													...
													access="hasAnyAuthority('ADMIN', 'COMERCIOS_SU', 'COMERCIOS_CU')">
													<a
														href="<c:url value='/comercios-${comercio.idComercio}-update'/>"
														class="btn-floating btn-medium waves-effect waves-light indigo"
														class=""><i class="material-icons">edit</i></a>
												</sec:authorize> &nbsp; <sec:authorize
													access="hasAnyAuthority('ADMIN', 'COMERCIOS_SU')">
													<a id="enlace"
														class="btn-floating btn-medium waves-effect waves-light  red"
														onClick="return eliminar('${comercio.descripcion}', '${comercio.idComercio}');"
				...
				<c:if test="${comercios.size()==0}">
					<div style="text-align: center">
						<p style="color: red; font: bold;">No existen comercios
							cargados</p>
					</div>
					<br>
					<br>
  ...
  function eliminar(comercio, id) {
 
	  
	swal({
	  title: 'Estás seguro?',
	  text: "El comercio '" + comercio + "' será eliminado",
	  type: 'warning',
  ...
  function(isConfirm){
		  if (isConfirm) {
				...
				var pos = URLactual.indexOf("comercios-listado");
				URLactual = URLactual.substring(pos, 0);
			  	window.location.href = URLactual + 'comercios-' + id +'-delete';
		  } 
	});
